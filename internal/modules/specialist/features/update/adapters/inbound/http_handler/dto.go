...
func ToUpdateSpecialistDTO(id string, req UpdateSpecialistRequest) application.UpdateSpecialistDTO {
	...
	if len(req.Keywords) > 0 {
		dto.Keywords = append([]string(nil), req.Keywords...)
	}

	if req.Status != nil {
		status := domain.SpecialistStatus(*req.Status)
		dto.Status = &status
	}
...
type UpdateSpecialistSuccessResponse struct {
